class DeclaratieFirmaService:
    def __init__(self, __repo):
        self.__repo = __repo

    def add(self, declaratie):
        declaratie_found = self.__repo.getOne(declaratie.id)
    ...
    def get_with_student_id(self, student_id):
        declaratie = self.__repo.get_with_student_id(student_id)
        if declaratie is None:
            raise ValueError("A declaration with the given student ID does not exist!")
        return declaratie
    ...
    def update(self, declaratie):
        declaratie_found = self.__repo.get_with_student_id(declaratie.student_id)

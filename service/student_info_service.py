class StudentInfoService:
    ...
    def add(self, student_info):
        student_info_found = self.__repo.getOne(student_info.id, student_info.id)
        if student_info_found is not None:
            raise ValueError("Already exists a user with given id")
        return self.__repo.add(student_info, student_info)
    ...
    def get_by_student_id(self, student_id):
        student_info = self.__repo.get_by_student_id(student_id, student_id)
        if student_info is None:

class DeclaratieUBBRepository:
    def getAll(self):
        from domain.declaratie_ubb import DeclaratieUBB
        declaratii = DeclaratieUBB.query.all()
        return declaratii

    def getOne(self, id):
        from domain.declaratie_ubb import DeclaratieUBB
        declaratie = DeclaratieUBB.query.get(id)
        return declaratie

    def get_with_student_id(self, student_id):
        from domain.declaratie_ubb import DeclaratieUBB
        declaratie = DeclaratieUBB.query.filter_by(student_id=student_id).first()
        return declaratie
